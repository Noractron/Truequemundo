...
function updateEstado(req, res) {
	if (req.body.email) {
		MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, async (err, db) => {
			...
			try {
				await coleccion.updateOne(condicional, newValues, (error, evento) => {
					res.send("ok");
				})
			} catch (e) {
				console.log(e);
...
function contadorVistas(req, res) {
	...
	MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, async (err, db) => {
		...
		try {
			...
			await coleccion.findAndModify(query, sort, operador, options, (error, eventos) => {
				res.send("ok");
			});
		} catch (error) {
			res.send(error);
...
function buscarProductoUsuario(req, res) {
	...
	if (req.body.email) {
		...
		MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, async (err, db) => {
			...
			await coleccion.find(query).toArray((error, eventos) => {
				if (err) throw error;
...
function allProductos(req, res) {
	...
	MongoClient.connect(url, { useNewUrlParser: true, useUnifiedTopology: true }, async (err, db) => {
		var base = db.db("truequeMundo");
		var coleccion = base.collection("producto");
		await coleccion.aggregate(query).toArray((error, eventos) => {
